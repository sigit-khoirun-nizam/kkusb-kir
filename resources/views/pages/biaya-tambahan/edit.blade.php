  ...
  <div class="space-y-6 max-w-xl mx-auto">
    ...
    <div class="flex items-center justify-between">
      <div>
        ...
        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Edit jenis biaya tambahan</p>
      </div>
    ...
    <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-white/[0.03]">
      <form action="{{ route('biaya-tambahan.update', $feeType) }}" method="POST">
        @csrf
        @method('PUT')
        <div class="space-y-6">
          <!-- Nama Biaya -->
          <div>
            ...
            <input type="text" name="name" value="{{ old('name', $feeType->name) }}" required placeholder="Contoh: STIKER PEMANTUL"
          ...
          <div>
            ...
            <div x-data="{ isOptionSelected: true }" class="relative z-20 bg-transparent">
              <select name="status" required
                ...
                <option value="aktif" {{ old('status', $feeType->status) == 'aktif' ? 'selected' : '' }} class="text-gray-700 dark:bg-gray-900 dark:text-gray-400">Aktif</option>
                <option value="nonaktif" {{ old('status', $feeType->status) == 'nonaktif' ? 'selected' : '' }} class="text-gray-700 dark:bg-gray-900 dark:text-gray-400">Nonaktif</option>
              </select>
